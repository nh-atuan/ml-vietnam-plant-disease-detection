...
| DEC-08 | Remove raw model label, latency, and prediction UUID from normal user-facing views. | These fields remain in API responses and server telemetry for debugging/grading if needed, but they should not compete with disease name, confidence, top-k, and recommendations in the demo UI. |
...
- Hide prediction UUID, latency, and raw model label from the default result view.
