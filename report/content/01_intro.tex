...
\begin{itemize}
    \item Đầu vào:Ảnh lá cây chụp ngoài thực địa (có thể chứa nhiều lá, nhiều vùng bệnh).
    \item Đầu ra:Mask pixel-wise xác định \emph{vùng bị bệnh} kèm theo \emph{nhãn loại bệnh} tương ứng.
    \item Phạm vi:8 lớp nhãn được chia đều cho hai đối tượng cây trồng chính:
          \begin{itemize}
              \item Lúa (Rice):\texttt{Healthy} (Khỏe mạnh), \texttt{BrownSpot} (Đốm nâu), \texttt{Hispa} (Sâu gai), \texttt{LeafBlast} (Đạo ôn).
              \item Cà phê (Coffee):\texttt{AlgalLeafSpot} (Đốm lá tảo), \texttt{LeafMiner} (Sâu vẽ bùa), \texttt{PowderyMildew} (Phấn trắng), \texttt{Rust} (Gỉ sắt).
          \end{itemize}
\end{itemize}
