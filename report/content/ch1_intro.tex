...
\begin{itemize}
  ...
  \item Nghiên cứu, huấn luyện và đánh giá so sánh hiệu năng của 5 mô hình phân vùng thực thể hiện đại (Mask R-CNN, YOLO26-seg phiên bản Nano, RF-DETR, MobileSAM, Mask2Former) trên tập dữ liệu đã xây dựng.
...
\begin{itemize}
  ...
  \item Pha 2 (Các bước 4, 5, 6): Huấn luyện độc lập 5 mô hình (Mask R-CNN, YOLO26-seg, RF-DETR, MobileSAM, Mask2Former) trên GPU T4 của Kaggle. Lựa chọn mô hình YOLO26-seg và tinh chỉnh siêu tham số bằng Ray Tune + ASHA, sau đó thực hiện lượng tử hóa sang định dạng ONNX phục vụ tối ưu hóa suy luận CPU.
