...
def _seed(element_id: str, salt: str = "seed") -> int:
    return zlib.crc32(f"{salt}:{element_id}".encode("utf-8")) & 0x7FFFFFFF
...
def build_application_architecture() -> dict:
    ...
    elements.extend(
        [
            text("app-title", 80, 38, 1440, "KIẾN TRÚC ỨNG DỤNG CHẨN ĐOÁN BỆNH LÁ CÂY", size=30, color=COLORS["title"]),
            text(
                "app-subtitle",
                240,
                86,
                1120,
                "Một yêu cầu ảnh đi qua giao diện, API điều phối và bốn dịch vụ chuyên trách",
                size=16,
                color=COLORS["body"],
            ),
            rectangle(
            ...
            text("service-region-label", 78, 458, 620, "TẦNG SUY LUẬN, DỮ LIỆU VÀ TRI THỨC", size=18, color=COLORS["title"], align="left"),
        ]
    )
    ...
    elements.append(text("user-text", 98, 226, 160, "Người dùng\nTrình duyệt", size=18))
    elements.extend(
        box_with_text(
            "nextjs",
            400,
    ...
    elements.extend(
        [
            ...
            text("onnx-arrow-label", 410, 392, 190, "Ảnh đã tiền xử lý", size=13, color=COLORS["ai_stroke"]),
            text("db-arrow-label", 650, 392, 170, "SQLModel / ORM", size=13, color=COLORS["primary_stroke"]),
            text("minio-arrow-label", 945, 392, 150, "Lưu / đọc ảnh", size=13, color=COLORS["warning_stroke"]),
            text("kb-arrow-label", 1160, 392, 210, "Tra cứu khuyến nghị", size=13, color=COLORS["success_stroke"]),
        ]
    )
    ...
    elements.extend(
        [
            text(
                ...
                880,
                660,
                "Kết quả trả về: nhãn bệnh + độ tin cậy + Top-K + URL ảnh + khuyến nghị",
                size=16,
                color=COLORS["body"],
            ),
...
def build_deployment_architecture() -> dict:
    ...
    elements.extend(
        [
            text("deploy-title", 80, 30, 1760, "KIẾN TRÚC TRIỂN KHAI GCP / k3s", size=30, color=COLORS["title"]),
            text(
                "deploy-subtitle",
                280,
                76,
                1360,
                "Từ GitHub Actions và GHCR tới Traefik, workload ứng dụng và lưu trữ bền vững",
                size=16,
                color=COLORS["body"],
            ),
            rectangle("gcp-boundary", 330, 245, 1510, 920, fill=COLORS["white"], stroke=COLORS["primary_stroke"], dashed=True, stroke_width=2),
            text("gcp-label", 355, 262, 330, "GCP VM • Ubuntu 22.04 LTS", size=20, color=COLORS["title"], align="left"),
            rectangle("k3s-boundary", 375, 310, 1420, 810, fill=COLORS["white"], stroke=COLORS["secondary_stroke"], dashed=True, stroke_width=2),
            text("k3s-label", 400, 327, 360, "k3s • containerd • Helm", size=18, color=COLORS["subtitle"], align="left"),
            rectangle("namespace-boundary", 420, 375, 1330, 690, fill=COLORS["white"], stroke=COLORS["body"], dashed=True, stroke_width=1),
            text("namespace-label", 445, 392, 430, "namespace: plant-disease", size=17, color=COLORS["body"], align="left"),
        ]
    )
    ...
    elements.extend(box_with_text("github-actions", 330, 115, 270, 110, "GitHub Actions\nLint • Test • Build", fill=COLORS["primary_fill"], stroke=COLORS["primary_stroke"], size=18))
    elements.extend(box_with_text("ghcr", 740, 115, 240, 110, "GHCR\nDocker images", fill=COLORS["ai_fill"], stroke=COLORS["ai_stroke"], size=18))
    elements.extend(box_with_text("helm", 1100, 115, 270, 110, "Helm qua SSH\nUpgrade • Rollout", fill=COLORS["success_fill"], stroke=COLORS["success_stroke"], size=18))
    elements.extend(evidence("pipeline-proof", 1445, 105, 360, "deploy.yml\nbuild → push → deploy → smoke test", size=14))
    ...
    elements.append(ellipse("internet-user", 30, 485, 175, 110, fill=COLORS["start_fill"], stroke=COLORS["start_stroke"]))
    elements.append(text("internet-user-text", 50, 517, 135, "Người dùng\nInternet", size=18))
    elements.extend(box_with_text("duckdns", 300, 485, 125, 110, "DuckDNS\nHTTPS", fill=COLORS["decision_fill"], stroke=COLORS["decision_stroke"], size=16))
    elements.extend(box_with_text("traefik", 515, 475, 240, 130, "Traefik Ingress\nTLS • định tuyến", fill=COLORS["decision_fill"], stroke=COLORS["decision_stroke"], size=18))
    elements.extend(evidence("route-proof", 515, 630, 240, "/        → frontend:3000\n/api/v1 → backend:8000\n/docs    → backend:8000", size=13))

    # Application workloads.
    elements.extend(box_with_text("frontend-pod", 870, 435, 270, 120, "Frontend Deployment\nNext.js • port 3000", fill=COLORS["primary_fill"], stroke=COLORS["primary_stroke"], size=18))
    elements.extend(box_with_text("backend-pod", 870, 605, 270, 135, "Backend Deployment\nFastAPI + ONNX Runtime", fill=COLORS["success_fill"], stroke=COLORS["success_stroke"], size=18))
    elements.extend(evidence("backend-proof", 870, 765, 270, "/health • /api/v1\nMODEL_PATH=/models/yolo26_quantized.onnx", size=13))
    ...
    elements.extend(box_with_text("postgres-workload", 1260, 445, 300, 120, "PostgreSQL StatefulSet\nDữ liệu người dùng và lịch sử", fill=COLORS["primary_fill"], stroke=COLORS["primary_stroke"], size=17))
    elements.extend(box_with_text("minio-workload", 1260, 610, 300, 120, "MinIO StatefulSet\nẢnh lá và object URL", fill=COLORS["warning_fill"], stroke=COLORS["warning_stroke"], size=17))
    elements.extend(box_with_text("redis-workload", 1260, 775, 300, 120, "Redis StatefulSet\ndịch vụ đã triển khai", fill=COLORS["secondary_fill"], stroke=COLORS["secondary_stroke"], size=17))
    ...
    elements.extend(box_with_text("model-pvc", 825, 930, 285, 100, "Model PVC • 10 GiB\nmount: /models", fill=COLORS["ai_fill"], stroke=COLORS["ai_stroke"], size=16))
    elements.extend(box_with_text("postgres-pvc", 1260, 930, 260, 100, "PostgreSQL PVC\n20 GiB", fill=COLORS["primary_fill"], stroke=COLORS["primary_stroke"], size=16))
    elements.extend(box_with_text("minio-pvc", 1540, 930, 180, 100, "MinIO PVC\n30 GiB", fill=COLORS["warning_fill"], stroke=COLORS["warning_stroke"], size=16))

    elements.extend(
        [
            text("runtime-note", 470, 1080, 1220, "Runtime đã xác minh: node Ready • workload Running • Ingress 80/443 • Helm deployed", size=16, color=COLORS["success_stroke"]),
