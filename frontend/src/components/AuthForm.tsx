...
const fieldHints: Record<AuthField, string> = {
  username: "Dùng từ 3 ký tự trở lên.",
  email: "Nhập địa chỉ email đang sử dụng.",
  password: "Dùng từ 6 ký tự trở lên.",
};
...
export default function AuthForm({ onSuccess, auth, notice }: AuthFormProps) {
  ...
  const validate = () => {
    ...
    if (username.trim().length < 3) {
      nextErrors.username = "Tên đăng nhập phải chứa ít nhất 3 ký tự.";
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        setFormError("Không thể hoàn tất xác thực. Vui lòng thử lại.");
      }
    } finally {
  ...
  return (
    <div className="w-full space-y-5">
      <div className="flex border-b border-surface-border" aria-label="Chọn hình thức xác thực">
        <button
          type="button"
      ...
      <form
        ...
        aria-label={isLoginMode ? "Biểu mẫu đăng nhập" : "Biểu mẫu đăng ký"}
      >
        <div>
        ...
        >
          {isSubmitting ? (
            <>
              ...
              <span aria-live="polite">Đang xử lý thông tin xác thực…</span>
            </>
          ) : isLoginMode ? (
            "Đăng nhập"
          ) : (
