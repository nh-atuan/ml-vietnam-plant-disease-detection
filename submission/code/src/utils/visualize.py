"""Visualization and image-analysis utilities for EDA."""
...
def setup_plot_style() -> None:
    ...
    Call once at the top of a notebook or script before any plotting.
    """
...
def sample_color_stats(
    ...
    Brightness uses the ITU-R BT.601 luminance formula:
        L = 0.299 R + 0.587 G + 0.114 B

    Contrast is estimated as the standard deviation of L over all pixels.

    Args:
        data_dir:   root directory that contains one sub-folder per class.
        classes:    ordered list of display class names.
        folder_map: {display_name: folder_name} mapping.
    ...
    for cls in classes:
        ...
        files  = [f for f in (data_dir / folder).iterdir()
...
def show_sample_grid(
    ...
    Args:
        data_dir:   root directory that contains one sub-folder per class.
        classes:    ordered list of display class names (determines row order).
        folder_map: {display_name: folder_name} mapping.
        palette:    list of hex colors (one per class) used for row labels.
        title:      figure super-title.
        n_col:      number of sample images to show per class.
        seed:       random seed for reproducible sampling.
    """
    rng = random.Random(seed)
    n_rows = len(classes)
    ...
    for row, (cls, color) in enumerate(zip(classes, palette)):
        ...
        for col in range(n_col):
            ...
            if row == 0:
                axes[row, col].set_title(f"Mẫu {col + 1}", fontsize=10, pad=4)
...
def compute_mean_histogram(
    ...
    Each channel histogram h_c is normalised so that sum(h_c) = 1,
    then averaged across all sampled images.
    ...
    Returns:
        Dict {"R": array(256,), "G": array(256,), "B": array(256,)}
        of mean normalised histograms.
    """
    rng    = random.Random(seed)
    folder = folder_map.get(cls, cls)
    files  = [f for f in (data_dir / folder).iterdir()
